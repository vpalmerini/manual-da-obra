...
router.post("/:id/systems", [
  ...
  try {
    ...
    const construction = await ConstructionService.addSystem(req.params.id, system._id);
...
router.delete("/:id/systems/:nickname", AuthMiddleware, async (req, res) => {
  try {
    const { id, nickname } = req.params;
    const system = await SystemService.deleteSystem({ construction: id, nickname })
    if (!system) {
      return res.status(404).json({
        status: 404,
      });
