...
const DetailSystem: React.FC<RouteComponentProps<RouteParams>> = ({
  history,
  match,
}) => {
  ...
  const getSystem = (id: string, nickname: string) => {
    ...
    detail(id, nickname)
      .then((response) => {
        const { name, description, files, construction } = response.data.system;
        setName(name);
              ...
              <div className={styles.cards}>
                {files && files.length > 0 ? (
                  ...
                  <h4 style={{ color: imageURL ? "#fff" : "#444" }}>
                    Este sistema não possui nenhum arquivo
                  </h4>
                )}
              </div>
            </div>
